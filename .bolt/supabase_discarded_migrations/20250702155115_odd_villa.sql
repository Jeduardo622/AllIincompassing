/*
# Add therapist role assignment function

1. New Functions
   - `assign_therapist_role` - Function to assign therapist role to a user

2. Purpose
   - Allows assigning a user to be associated with a specific therapist
   - Creates appropriate user_roles entries
*/
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      IF NOT EXISTS (
        ...
        WHERE tablename = 'user_therapist_links' AND policyname = 'Admins can manage user-therapist links'
      ) THEN
  ...
  -- Set user metadata (this is now handled by the Auth API in the calling function)
END;
$$;
