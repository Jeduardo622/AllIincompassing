...
The only narrow exceptions are the three exact workflow paths in the allowlist above. After the owner personally inspects and merges the critical PR, the owner may explicitly authorize Codex in the current task to perform exactly one browser click dispatch through the owner's already-authenticated in-app GitHub browser session. Authorization is separate per workflow and requires fresh current-task owner authorization per workflow.
