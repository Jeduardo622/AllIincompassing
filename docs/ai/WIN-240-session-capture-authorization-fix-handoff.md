...
- Protected runtime project `wnnjeqheqxxyrgsjmygy` is healthy. The checked-in `20260722181239_resolve_assigned_bt_session_capture_billing.sql` migration was applied through the Supabase connector after explicit approval; the hosted ledger recorded it as `20260722192000 / resolve_assigned_bt_session_capture_billing`. Live function/grant inspection confirms `SECURITY DEFINER`, an empty `search_path`, no `anon`/`public` execute grant, and the intended `authenticated` execute grant.
