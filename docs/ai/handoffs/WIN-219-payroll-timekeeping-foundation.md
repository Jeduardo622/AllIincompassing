...
- Hosted root cause: the credentialed `playwright:session-no-show` gate reached the Start Session action, but the new payroll lifecycle correctly withheld `sessions-start` because the protected `payroll-time-events` Edge authority is not deployed before owner activation. The live gateway returned HTTP 404 with the exact Supabase `NOT_FOUND` / `Requested function was not found` payload, and the production database still lacks the payroll foundation tables by design.
- Bounded repair: authenticated, organization-scoped `get_session_context` requests map only that exact gateway response to the existing `feature_disabled` contract. Clinical start remains independently authorized by `sessions-start`. All other 404s, malformed responses, authorization failures, and payroll errors continue to fail closed.
- TDD proof: the new exact missing-function test failed with 404 before implementation, then the payroll handler suite passed 21/21. The broader payroll API, lifecycle domain, and Playwright lifecycle helper batch passed 104/104. Policy, tenant validation, lint, typecheck, build, and Tier-0 routes passed; the exact 12 GB/four-worker `npm run verify:local` wrapper exited zero in 605.2 seconds with 92.96 percent line coverage and 244/244 Tier-0 routes.
