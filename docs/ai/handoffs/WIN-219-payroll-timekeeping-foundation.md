...
- Hosted gate: the new migration is not yet applied to hosted Supabase. Hosted application, if separately permitted at the review-ready exact commit, must verify the migration is absent, apply only the reviewed function replacement, and prove the exact function body, ACL, feature-disabled behavior, unchanged enabled-mode authority, payroll-disabled state, and no operational/capability rows. The manager-assignment advisor migration also remains unapplied.
