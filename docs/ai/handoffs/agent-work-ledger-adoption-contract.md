...
- commits: design/plan `8a24345f`, implementation `71a6a517`, security/reliability hardening `59cbc580`, evidence binding `00801f57`, fail-closed partial evidence `0ed7d71d`, and failure-code preservation `fe73ceae`
...
- `npm run agent-work:local:operator` is a no-argument wrapper over the existing Phase 2 harness. It rejects caller-selected scope, requires the exact 12-check manifest and cleanup matrix, and validates persisted successful or failed manifests against the sanitized in-memory result before output.
...
- the observer blocks non-read methods and external origins, records no storage/HAR/trace/video or network bodies, persists only full opaque SHA-256 route identities, removes all artifacts from an invocation if any observation or write throws, and applies layout redaction after measurement and before screenshot capture so text, carets, media, SVG/canvas content, and background imagery cannot enter retained images
...
- GREEN: focused operator, Phase 2 harness, unit, and real-browser suites pass `99/99`, including local URL/port rejection, traversal rejection, full-digest route binding, persisted failed-manifest validation, both fixed viewports, non-interactive label exclusion, canonical failure codes, atomic artifact cleanup, and same-origin mutation blocking
...
- final synthetic observer command passed both viewports from commit `fe73ceae` with opaque route ID `sha256:13b6902d199056c8c067ff3972db79f6d309fed7a8baf9db3231c12ed3001fae`; the temporary fixture and stale predecessor artifacts were removed
- redacted desktop screenshot/evidence hashes: `sha256:92fa1c35ab4c5db9efcf12e327eb46e47fb063b914844641ff19ccaf070ba631` / `sha256:60aef945c3c9b4258932bf3658eeafb78c2dda2a0ca851b3d9b9f3f406e76a9c`
- redacted mobile screenshot/evidence hashes: `sha256:5368b716d05dcc7955feffac11e228a8272717c062bcb18dc90fc32dbd374d19` / `sha256:936821aac026914b57dd879b98b55531748137623bcc75c4248c70e791ca23cc`
- visual inspection confirmed both retained screenshots contain layout geometry only, with no text or media
- intermediate operator run `20260806T224302Z-cdc45b` failed after `705494ms` at `reset_retention-trace_failed_timeout` and cleanup audit; it was not counted as passing, and residue inspection found no retained container/network/volume state
- decisive operator run `20260806T225600Z-5cdff9` passed all 12 checks and cleanup from commit `0ed7d71dd3df8079ce24ef4b0e10e6a6a88b23c8` in `668117ms`; image `sha256:5b665ec5b1e4c4a5446b4e8341ac342a7b0c49e9ffe6e08309237b3774e959f5`; summary hash `685c86155c875e298b5159eed5fd2736d1199fc6d97e93c27747e956ca44a28d`; check-evidence hash `a8456301455722cfcedd31b466a63d41e5d56e7385618432adb2fbbb98a78d1a`
- the first full `test:ci` run reached `4074` passes and failed only the two expected protected-doc hash checks; after the additive attestation refresh, both focused hash-contract suites passed `43/43`
- final critical matrix passed in `222.6s`: policy, lint, typecheck, 8 GB full `test:ci` with coverage, tenant validation, and build all exited `0`
- final `NODE_OPTIONS=--max-old-space-size=8192 npm run verify:local` passed in `375.3s`, including policy, lint, typecheck, full tests/coverage, build, and Tier-0 routes `220/220`
- expected mocked AI-documentation `ECONNREFUSED` stderr remained non-failing and made no provider call; auth/session Playwright was not rerun because this slice changes no auth, session, or route behavior
...
- architecture/security reviews found raw screenshots, raw route text, truncated path identity, partial-artifact residue, unstable fatal output, and a real thrown-failure path that bypassed validation; screenshots are now layout-redacted, filenames use the full route digest, failed invocations remove their artifacts, fatal output has exactly three top-level keys, and both successful and failed persisted manifests are reread and validated before output
