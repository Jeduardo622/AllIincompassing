...
- machine approval gate: dispatch must use the `main` workflow ref, identify the merged WIN-275 PR whose merge commit is the immutable current `main` head, and find a current-head `APPROVED` review from an independent human GitHub user. Owner acknowledgement alone is insufficient.
...
- review repair: `docs/ai/reviews/WIN-275-hosted-shadow-proof-attestation.md` binds the exact merged implementation surfaces and hashes to a successor PR. That PR must receive a current-head independent human approval and merge as the current `main` head before owner dispatch.
...
- next action: obtain an independent human approval on the current trace-scope repair PR, merge it while current, then owner-dispatch the protected workflow using that PR number and exact merge SHA. Verify the sanitized artifact plus final disabled zero-residue state. Do not authorize `advisory` or `active`.
