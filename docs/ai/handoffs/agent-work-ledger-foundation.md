...
- next action: obtain an independent human approval on the successor hash-bound attestation PR, merge it while current, then owner-dispatch the protected workflow using the successor PR number and exact merge SHA. Verify the sanitized artifact plus final disabled zero-residue state. Do not authorize `advisory` or `active`.
