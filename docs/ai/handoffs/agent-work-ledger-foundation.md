...
- next action: publish the reviewed branch as a WIN-275 critical-lane PR, pass exact-head CI, obtain independent human approval, merge while it remains current `main`, then owner-dispatch the protected workflow and verify its sanitized artifact plus final disabled zero-residue state. Do not authorize `advisory` or `active`.
