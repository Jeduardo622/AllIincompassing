...
- Forward migration `20260711143506_validate_goal_target_trial_measurement_payloads.sql` enforces response/value shape at the `trial_events` table boundary for every insert/update, so direct authenticated finalizer calls cannot bypass measurement-type validation.
