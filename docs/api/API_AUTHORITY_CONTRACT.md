...
- Local entrypoint verification runs with `npm run agent-work:edge-smoke`. The user-facing `agent-work-items` function keeps `verify_jwt = true` and re-verifies the caller through `getUser`. The machine-only runner and sweeper use `verify_jwt = false` because Supabase publishable keys are not JWTs; each handler instead requires both a project publishable `apikey` and its distinct invocation secret before any queue read or RPC. Their internal service-role clients remain environment-only and are never derived from request headers.
