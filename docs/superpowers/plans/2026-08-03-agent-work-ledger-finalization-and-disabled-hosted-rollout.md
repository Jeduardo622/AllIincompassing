...
This is the only remaining execution slice. Trust the committed evidence and do not repeat architecture exploration or completed Tasks 1-4. Start from branch `codex/agent-work-ledger-foundation` at committed HEAD `d89ea1d7e5f96940a362f04c1a48577ef904d888`; verify live state before acting because the branch may have advanced after this checkpoint.

The local integration harness remains local-only. Do not deploy `scripts/agent-work-ledger-harness/**`, `docker/agent-work-ledger/**`, the Phase 2 Compose stack, `enable_local_agent_work_queue_scheduler`, fixed local Vault names, or `host.docker.internal` callbacks. Hosted rollout consists only of committed migrations, the six reviewed Edge Functions, process-injected non-secret/runtime configuration, disabled-mode Queue/Cron/Vault configuration, and sanitized synthetic smoke.

Expected entry drift:

- intended uncommitted privacy closure: `supabase/migrations/20260801090000_agent_work_ledger_core.sql`, `scripts/agent-work-ledger-security-contract.mjs`, and `tests/agentWorkLedgerApprovalMigration.test.ts`
- unrelated byte-preserved drift that must remain unstaged: `deno.lock` SHA256 `29B1C7A80798784390097AE00F90FAFEA8C73E6182E77D17EDA503DE458EC52E` and `reports/test-reliability-latest.json` SHA256 `934C3A7A50F5E6CFDF3F27714922DB090F5E466A35784F1A1B9A5B7FCE378B49`
- recovery stashes that must remain untouched: `task9-recheckpoint-before-authority-followup` and `task9-checkpoint-before-api-convergence-repair`

Complete these boundaries in order:

- [ ] **Close authenticated base-table reads through TDD.** Keep direct `authenticated` grants revoked. Add a RED static/runtime contract, then update `agent-work-items` so caller-bound RPCs establish work-item or assessment visibility before narrowly scoped service-role reads. Keep `auth.getUser`, `current_user_can_read_agent_work_item_endpoint`, `current_user_decidable_agent_work_approval_ids`, and actor-aware scope resolution on the authenticated boundary. Never replace the authorization boundary with an unconditional service-role read.
- [ ] **Prove and commit the privacy closure.** Run focused Vitest and Deno handler tests, fresh `agent-work:db:reset`, `agent-work:security-contract`, and `agent-work:edge-smoke`. Confirm same-tenant list/detail/mutation behavior, cross-tenant denial, sanitized DTOs, and zero provider calls. Commit only the privacy-closure files as `fix(agent-work): close direct ledger table reads`.
- [ ] **Freshly route and repeat independent final review.** Route `high-risk human-reviewed` / `critical`. Run code-review, test, security, Supabase, architecture, DevOps/performance, documentation, clinical, product, and privacy lanes in parallel. Agent review supplies findings and evidence but cannot substitute for mandatory human PR approvals. Resolve only actionable in-scope findings through narrowly routed RED/GREEN fixes and focused commits.
- [ ] **Run the complete final local gate once from committed state.** Restore the two unrelated drift files from their exact recovery copies after commands that rewrite them. Run affected checks, then `NODE_OPTIONS=--max-old-space-size=8192 npm run verify:local`, `git diff --check`, and two serial `npm run test:agent-work:phase2` runs. Require cleanup audit pass after each run. Record every skip, failure, retry, timeout, count, image/hash, and unrun check exactly.
- [ ] **Close local evidence.** Update this plan's checkboxes and `docs/ai/handoffs/agent-work-ledger-foundation.md` with current HEAD, routes, commits, specialist dispositions, verification card, PR-hygiene verdict, two Phase 2 runs, `policy_unapproved` retention, exact drift hashes, and the hosted smoke boundary. Commit only the handoff/plan closure.
- [ ] **Push and open the PR.** Update WIN-271 first, then push and create the PR using the handoff as the body. Inspect authoritative GitHub checks, branch protection, and reviews. Poll every three minutes for at most 45 minutes. Fix only in-scope failures, rerun affected gates, and update Linear/handoff.
- [ ] **Merge only at the human gate.** Require recorded human protected-path, Supabase, security, clinical, product, and privacy approvals, all required checks passing, no unresolved requested changes, and branch protection permitting merge. If any approval is unavailable after bounded polling, stop with the exact reviewer/check blocker; this is the only expected reason the slice may end review-ready instead of merged.
- [ ] **Perform the disabled hosted Supabase rollout after merge.** Freshly route the hosted boundary. Through the Supabase connector, confirm the exact project, compare local/remote migration history, inspect a dry run, and apply only committed ledger migrations. Fail closed on drift or project mismatch. Configure reviewed process-injected secrets/configuration without reading or writing `.env*`; set runtime to `disabled`. Deploy `agent-work-items`, `agent-work-runner`, `agent-work-sweeper`, `agent-trace-report`, `ai-agent-optimized`, and `generate-program-goals`. Verify JWT policy, RLS/grants, tenant isolation, Queue, Cron, Vault, runner/sweeper health, runtime-policy denial, and sanitized synthetic smoke. Do not make a successful provider-backed generation call; local dependency-injected fakes are the positive continuity proof and hosted smoke covers pre-provider denial paths.
- [ ] **Release Netlify only after disabled Supabase health is proven.** Freshly route Netlify, release the merged commit, and run redacted tenant/app/API/assessment smoke. Keep runtime `disabled`. `shadow` or `advisory` requires a separately recorded human decision; `active` is forbidden.
- [ ] **Finish operational hygiene.** Update WIN-271 and the handoff with PR/merge/deployment links, live checks, migration/function/config state, sanitized smoke evidence, retention decision, residual risks, and all blocked/unrun work. Confirm the feature branch/worktree and both recovery stashes remain recoverable until post-merge cleanup is explicitly safe.

Completion means the merged commit is deployed and healthy in hosted `disabled` mode, Netlify serves that commit, mandatory human approvals and live checks are recorded, retention deletion remains `policy_unapproved`, and no local harness component or external model call was promoted. If the mandatory human gate does not clear, completion for this slice is a fully review-ready PR plus the exact external blocker; do not perform hosted rollout before merge.
