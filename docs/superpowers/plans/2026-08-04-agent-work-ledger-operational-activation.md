...
PR `#897` merged as `f053fa4562e8d51a2d984ee2627bd7a8a863005f` after exact-head CI passed but without a submitted independent human approval. It is a process exception and cannot qualify for dispatch. The hash-bound successor review in `docs/ai/reviews/WIN-275-hosted-shadow-proof-attestation.md` must receive an independent current-head human approval and merge as the current `main` head; only that successor PR number and merge SHA may be used for the protected dispatch.

PR `#898` then merged as `671c8dc80c3610f7924af9393e836291ca48c7f4` without that approval and is also ineligible. A subsequent read-only hosted audit found the Ledger and queues empty but `55,890` historical rows in the shared `agent_execution_traces` table. The trace-scope repair keeps Ledger-owned surfaces globally zero while every shared-trace proof, cleanup, and residue check matches either synthetic organization or exact synthetic work item; preflight uses NIL work-item placeholders. Attempt/effect/draft-packet assertions remain synthetic-organization scoped. The repair PR itself must receive the independent current-head approval before it can become the dispatch anchor.
