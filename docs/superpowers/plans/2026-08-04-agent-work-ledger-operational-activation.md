...
The hosted shadow proof follow-on is owner-dispatched, shadow-only, and still requires human review. It uses an immutable current `main` SHA, exact-head passing CI, synthetic fixtures, and mandatory cleanup with redundant `disabled` restoration. Independent-human approval remains the default. When GitHub proves exactly one GitHub human maintainer with write-or-higher access in this personal repository, the `solo-maintainer owner-attested critical lane` may substitute an explicit owner decision backed by hash-bound code, security, test, and Supabase agent reviews. Merge and dispatch remain separate owner actions, and any authority ambiguity fails closed.
...
PR `#897` merged as `f053fa4562e8d51a2d984ee2627bd7a8a863005f` after exact-head CI passed but without a submitted independent human approval. It is a process exception and cannot qualify for dispatch. The hash-bound successor review in `docs/ai/reviews/WIN-275-hosted-shadow-proof-attestation.md` remains historical evidence; only a fresh PR satisfying either the independent-human default or the fail-closed solo-owner contract may become the protected dispatch anchor.

PR `#898` then merged as `671c8dc80c3610f7924af9393e836291ca48c7f4` without that approval and is also ineligible. A subsequent read-only hosted audit found the Ledger and queues empty but `55,890` historical rows in the shared `agent_execution_traces` table. The trace-scope repair keeps Ledger-owned surfaces globally zero while every shared-trace proof, cleanup, and residue check matches either synthetic organization or exact synthetic work item; preflight uses NIL work-item placeholders. Attempt/effect/draft-packet assertions remain synthetic-organization scoped. Neither historical merge is retroactively eligible; only a fresh PR satisfying the independent or solo-owner route can become the dispatch anchor.
