...
7. One database transaction persists the structured form and signature attestation, locks the clinical note, applies the existing goal/target progression behavior, and changes the session from `in_progress` to `completed`.
...
- Add an RPC that finalizes the BT note and completes the session atomically. It must reuse current authority helpers, verify the caller-to-therapist link and organization, validate the current session status, and preserve the existing target-progression contract.
