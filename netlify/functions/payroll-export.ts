...
import { payrollExportHandler } from "../../src/server/api/payroll-export";
...
export const handler: Handler = async (event) => {
  try {
    ...
    const response = await payrollExportHandler(request);
    return toNetlifyResponse(response);
  } catch {
    const request = new Request("https://netlify.internal/api/payroll-export", {
      headers: event.headers as HeadersInit,
    });
