...
} from '../src/lib/smoke/serviceAccountProbe.ts';
...
const checkSupabaseServiceAccount = async (runtimeConfig: RuntimeConfig): Promise<void> => {
  const env = process.env as Record<string, string | undefined>;
  const config = resolveServiceAccountProbeConfig(env, {
    supabaseUrlOverride: runtimeConfig.supabaseUrl,
  });

  if (!config) {
    throw new Error('Service account smoke probe requires SUPABASE_SERVICE_ROLE_KEY to be set');
  }

  const result = await runServiceAccountSmokeProbe(config);
  if (!result.ok) {
    const message = result.error?.message ?? 'Unknown service account smoke failure';
    throw new Error(`Service account probe failed after ${result.durationMs}ms: ${message}`);
  }

  console.log('[smoke] Supabase service account listUsers -> OK', {
...
const run = async (): Promise<void> => {
  ...
  await checkSupabaseServiceAccount(runtimeConfig);
