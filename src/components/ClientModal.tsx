...
export default function ClientModal({
  ...
  const handleFormSubmit = async (data: ClientFormData) => {
    ...
    if (typeof window === 'undefined' || window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      fetch('http://127.0.0.1:7802/ingest/c639188e-4d1d-4ae2-8578-fbb07665dceb',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'27eafc'},body:JSON.stringify({sessionId:'27eafc',runId:'initial',hypothesisId:'H3',location:'src/components/ClientModal.tsx:208',message:'ClientModal submit produced formatted payload',data:{formattedKeys:Object.keys((formatted && typeof formatted === 'object') ? formatted as Record<string, unknown> : {}),hasTopLevelServiceContracts:Boolean(formatted && typeof formatted === 'object' && 'service_contracts' in (formatted as Record<string, unknown>)),hasInsuranceInfoServiceContracts:Boolean(formatted && typeof formatted === 'object' && formatted.insurance_info && typeof formatted.insurance_info === 'object' && !Array.isArray(formatted.insurance_info) && 'service_contracts' in (formatted.insurance_info as Record<string, unknown>)),serviceContractCount:Array.isArray((formatted as { service_contracts?: unknown }).service_contracts) ? ((formatted as { service_contracts?: unknown[] }).service_contracts?.length ?? 0) : null},timestamp:Date.now()})}).catch(()=>{});
    }
    // #endregion
    setLocalError(null);
    try {
