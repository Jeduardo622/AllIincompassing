...
const administrationReadSchema = z.object({
  ...
  if (!value.capabilities.canExportPeriod) {
    value.payPeriods.forEach((period, index) => {
      if (period.latestExport !== null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["payPeriods", index, "latestExport"],
          message: "Export details require payroll.export_period.",
        });
      }
    });
  }
