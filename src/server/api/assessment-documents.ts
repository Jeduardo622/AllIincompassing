...
const autoGenerateDraftsFromExtractedChecklist = async (args: {
  ...
  const checklistResult = await fetchJson<AssessmentChecklistValueRow[]>(
    `${supabaseUrl}/rest/v1/assessment_checklist_items?select=section_key,label,placeholder_key,value_text,value_json&organization_id=eq.${encodeURIComponent(
      organizationId,
  ...
  const createdProgramResult = await fetchJson<Array<{ id: string }>>(`${supabaseUrl}/rest/v1/assessment_draft_programs`, {
    method: "POST",
    ...
    body: JSON.stringify({
      ...
      organization_id: organizationId,
      client_id: clientId,
      name: generatedResult.data.program.name,
      description: generatedResult.data.program.description ?? null,
      rationale: generatedResult.data.rationale ?? null,
  ...
  const draftProgramId = createdProgramResult.data[0].id;
  const draftGoalsPayload = generatedResult.data.goals.map((goal) => ({
    assessment_document_id: createdDocumentId,
    draft_program_id: draftProgramId,
    organization_id: organizationId,
    client_id: clientId,
    title: goal.title,
