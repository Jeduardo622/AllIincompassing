...
const readResponseSchema = z.object({
  ...
  payPeriods: z.array(z.object({
    ...
    latestExport: z.object({
      runId: z.string().uuid(),
      adapterVersion: z.literal("provider-neutral-v1"),
...
const collectCompensationPaths = (value: unknown, path = "$"): string[] => {
  ...
  return Object.entries(value as Record<string, unknown>).flatMap(([key, nestedValue]) => {
    ...
    if (
      key === "hourlyRateCents"
      || key === "totalMealPremiumCents"
      || key === "totalGrossEarningsCents"
    ) {
      return [nextPath];
    }
...
const validateReadInvariants = (
  ...
  const allowedCompensationPaths = new Set<string>();
  if (payload.capabilities.canViewCompensation) {
    payload.employments.forEach((employment, index) => {
      if (employment.compensation !== undefined) {
        allowedCompensationPaths.add(`$.employments[${index}].compensation`);
        allowedCompensationPaths.add(`$.employments[${index}].compensation.hourlyRateCents`);
      }
    });
  }
  if (payload.capabilities.canExportPeriod) {
    payload.payPeriods.forEach((period, index) => {
      if (period.latestExport !== null) {
        allowedCompensationPaths.add(`$.payPeriods[${index}].latestExport.totalMealPremiumCents`);
        allowedCompensationPaths.add(`$.payPeriods[${index}].latestExport.totalGrossEarningsCents`);
      }
    });
  } else if (payload.payPeriods.some((period) => period.latestExport !== null)) {
    return payrollErrorResponse(request, 502, {
      code: "invalid_response",
      message: "Invalid payroll administration response.",
      classification: PAYROLL_ERROR_CLASSIFICATIONS.invalid_response,
