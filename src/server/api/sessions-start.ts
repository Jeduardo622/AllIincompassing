...
export async function sessionsStartHandler(request: Request): Promise<Response> {
  ...
  if (request.method !== "POST") {
    return errorResponse(request, "validation_error", "Method not allowed", { status: 405, headers: traceHeaders });
  }
  ...
  try {
    ...
    if (rateLimit.limited) {
      return errorResponse(request, "rate_limited", "Too many session start requests", {
    ...
    const sessionUrl = `${supabaseUrl}/rest/v1/sessions?select=id,client_id,organization_id,program_id,goal_id,therapist_id,status,started_at&organization_id=eq.${encodedOrganizationId}&id=eq.${encodedSessionId}`;
