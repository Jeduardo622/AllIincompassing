...
export const handlePrograms = async (req: Request) => {
  ...
  if (req.method === "GET") {
    ...
    if (!z.string().uuid().safeParse(clientId).success) return json({ error: "client_id must be a valid UUID" }, 400);
    ...
    if (error) return json({ error: "Failed to load programs" }, 500);
    return json(data ?? []);
  }
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await db
      ...
      .select("*")
      .limit(1);
    if (error) return json({ error: "Failed to create program" }, 500);
  ...
  if (req.method === "PATCH") {
    ...
    const { data, error } = await orgScopedQuery(db, "programs", orgId)
      ...
      .select("*")
      .limit(1);
    if (error) return json({ error: "Failed to update program" }, 500);
