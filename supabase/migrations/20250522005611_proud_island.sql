...
-- Simplify manage_admin_users function with better error handling
...
-- Add admin role to all existing users to ensure they have access
DO $$
DECLARE
  admin_role_id uuid;
  user_record RECORD;
BEGIN
  ...
  FOR user_record IN SELECT id, email FROM auth.users LOOP
    ...
    RAISE NOTICE 'Admin role added to %', user_record.email;
  END LOOP;
END $$;