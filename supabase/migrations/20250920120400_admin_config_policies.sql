...
/*
  Harden configuration table access (admin-only policies via app.user_has_role)
*/
...
DROP POLICY IF EXISTS "Admins manage service lines" ON service_lines;
...
DROP POLICY IF EXISTS "Admins manage referring providers" ON referring_providers;
...
DROP POLICY IF EXISTS "Admins manage file cabinet settings" ON file_cabinet_settings;
