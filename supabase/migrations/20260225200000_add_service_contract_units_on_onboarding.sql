...
COMMENT ON COLUMN public.service_contracts.authorized_units IS
  'Authorized units captured for the service contract.';
...
DECLARE
  ...
  v_status text;
  v_service_contract_provider_raw text;
  v_service_contract_provider text;
  v_service_contract_units_raw text;
  v_service_contract_units numeric := 0;
...
BEGIN
  ...
  v_service_contract_provider_raw := lower(trim(COALESCE(
    NULLIF(v_payload->>'service_contract_provider', ''),
    NULLIF(v_payload->'insurance_info'->>'service_contract_provider', ''),
    NULLIF(v_payload->'insurance_info'->>'provider', ''),
    ''
  )));

  IF v_service_contract_provider_raw = 'iehp' THEN
    v_service_contract_provider := 'IEHP';
  ELSIF v_service_contract_provider_raw IN ('caloptima', 'cal optima') THEN
    v_service_contract_provider := 'CalOptima';
  ELSE
    v_service_contract_provider := NULL;
  END IF;

  v_service_contract_units_raw := trim(COALESCE(
    NULLIF(v_payload->>'service_contract_units', ''),
    NULLIF(v_payload->'insurance_info'->>'service_contract_units', ''),
    ''
  ));

  IF v_service_contract_units_raw ~ '^\d+(\.\d+)?$' THEN
    v_service_contract_units := GREATEST(v_service_contract_units_raw::numeric, 0);
  END IF;
  ...
  IF v_service_contract_provider IS NOT NULL THEN
    ...
    VALUES (
      v_org,
      v_result.id,
      v_service_contract_provider,
      v_effective_date,
      v_termination_date,
      'ACH',
      v_service_contract_units,
      v_actor,
      v_actor
    );
  END IF;
