...
create index concurrently if not exists client_notes_client_created_desc_idx
  on public.client_notes (client_id, created_at desc);

create index concurrently if not exists client_notes_client_parent_created_desc_idx
  on public.client_notes (client_id, created_at desc)
  where is_visible_to_parent = true;

create index concurrently if not exists client_issues_client_created_desc_idx
  on public.client_issues (client_id, created_at desc);

create index concurrently if not exists sessions_org_created_desc_idx
