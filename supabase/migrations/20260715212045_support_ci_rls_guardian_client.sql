-- @migration-intent: Allow the service-only synthetic RLS profile guard to resolve an active guardian's tenant from canonical client links without weakening tenant checks.
-- @migration-dependencies: 20260715201001_support_ci_rls_linked_therapist.sql,20251226090000_client_guardians.sql
-- @migration-rollback: Forward recovery only. Restore a reviewed service-only provisioner that preserves guardian and client authority and fail-closed tenant validation.
...
declare
  ...
  client_authority_count integer := 0;
  client_organization_count integer := 0;
...
begin
  ...
  if resolved_role = 'therapist'::public.role_type then
    ...
    select
      count(*)::integer,
      count(distinct client_authority.organization_id)::integer,
      (array_agg(distinct client_authority.organization_id))[1]
    into
      client_authority_count,
      client_organization_count,
      resolved_organization_id
    from (
      ...
      where cg.guardian_id = p_user_id
        and cg.deleted_at is null
        and cg.organization_id = c.organization_id
        and c.deleted_at is null
    ) client_authority;

    if client_authority_count > 0 and client_organization_count <> 1 then
      raise exception using
        errcode = '42501',
        message = 'Synthetic RLS actor client mapping is ambiguous';
    end if;
