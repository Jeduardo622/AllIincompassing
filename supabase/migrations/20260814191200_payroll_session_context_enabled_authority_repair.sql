-- @migration-intent: payroll_session_context_enabled_authority_repair
-- @migration-dependencies: 20260814183500_payroll_session_context_disabled_precedence.sql
-- @migration-rollback: Restore the prior public.get_session_payroll_context(uuid) body from 20260814183500_payroll_session_context_disabled_precedence.sql, preserve grants, then reload the PostgREST schema.
...
begin
  ...
  if coalesce(v_feature_flag_enabled, false) is not true and v_actor_is_assigned_employee then
    return jsonb_build_object(
      'state', 'feature_disabled',
      'sessionId', v_session.id,
      'organizationId', v_actor_org
