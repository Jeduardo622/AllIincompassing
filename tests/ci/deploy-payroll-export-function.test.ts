...
const scriptPath = path.join(repoRoot, "scripts", "ci", "deploy-payroll-export-function.mjs");
...
const makeFakeSupabase = ({
  listPayload = [{ slug: "payroll-export", verify_jwt: true }],
  deployFailure = "rate-limit-once",
} = {}) => {
  const root = mkdtempSync(path.join(tmpdir(), "supabase-payroll-export-deploy-"));
...
describe("deploy-payroll-export-function", () => {
  afterEach(() => {
  ...
  test("deploys payroll-export, retries Docker rate limits with --use-api, and verifies verify_jwt", () => {
    ...
    expect(state.calls).toEqual([
      ["functions", "deploy", "payroll-export", "--project-ref", "wnnjeqheqxxyrgsjmygy"],
      [
        "functions",
        "deploy",
        "payroll-export",
  ...
  test("fails when payroll-export is missing from the remote function list", () => {
    const { root, statePath } = makeFakeSupabase({
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "Missing deployed function after deploy: payroll-export",
    );
  });

  test("fails when payroll-export verify_jwt is false", () => {
    const { root, statePath } = makeFakeSupabase({
      listPayload: [{ slug: "payroll-export", verify_jwt: false }],
      deployFailure: "none",
    });
    const result = spawnSync(process.execPath, [scriptPath], {
      cwd: repoRoot,
    ...
    expect(result.status).toBe(1);
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "verify_jwt must be true for payroll-export",
    );
  });
