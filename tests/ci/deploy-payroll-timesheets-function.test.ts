...
const scriptPath = path.join(repoRoot, "scripts", "ci", "deploy-payroll-timesheets-function.mjs");
...
const makeFakeSupabase = ({
  listPayload = [{ slug: "payroll-timesheets", verify_jwt: true }],
  deployFailure = "rate-limit-once",
} = {}) => {
  const root = mkdtempSync(path.join(tmpdir(), "supabase-payroll-deploy-"));
...
describe("deploy-payroll-timesheets-function", () => {
  afterEach(() => {
  ...
  test("deploys payroll-timesheets, retries Docker rate limits with --use-api, and verifies verify_jwt", () => {
    ...
    expect(state.calls).toEqual([
      ["functions", "deploy", "payroll-timesheets", "--project-ref", "wnnjeqheqxxyrgsjmygy"],
      [
        "functions",
        "deploy",
        "payroll-timesheets",
  ...
  test("fails when payroll-timesheets is missing from the remote function list", () => {
    const { root, statePath } = makeFakeSupabase({
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "Missing deployed function after deploy: payroll-timesheets",
    );
  });

  test("fails when payroll-timesheets verify_jwt is false", () => {
    const { root, statePath } = makeFakeSupabase({
      listPayload: [{ slug: "payroll-timesheets", verify_jwt: false }],
      deployFailure: "none",
    });
    const result = spawnSync(process.execPath, [scriptPath], {
      cwd: repoRoot,
    ...
    expect(result.status).toBe(1);
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "verify_jwt must be true for payroll-timesheets",
    );
  });
