...
describe('auth-signup identity throttle contract', () => {
  beforeEach(() => {
    ...
    rateLimitMock.mockImplementation((key: string) => {
      ...
      if (key.startsWith('auth-signup:identity:')) {
  ...
  it('returns the current 429 signup envelope with Retry-After when the identity throttle trips', async () => {
    ...
    expect(rateLimitMock).toHaveBeenNthCalledWith(1, 'auth-signup:ip:203.0.113.10', 10, 60_000);
    expect(rateLimitMock).toHaveBeenNthCalledWith(2, 'auth-signup:identity:user@example.com', 4, 60_000);
