...
async function loadHandler() {
  ...
  vi.doMock('../../supabase/functions/_shared/database.ts', () => ({
    ...
    supabaseAdmin: {
      rpc: vi.fn(async (fn: string) => {
        if (fn !== 'confirm_session_holds_batch_with_enrichment') {
  ...
  await import('../../supabase/functions/sessions-confirm/index.ts');

  if (!serveHandler) {
    throw new Error('Expected sessions-confirm to register a Deno.serve handler');
  }

  return serveHandler;
}

describe('sessions-confirm retry-after contract', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    vi.unstubAllGlobals();
  ...
  it('returns the current 409 conflict envelope with Retry-After for the first retryable sessions-confirm branch', async () => {
    const handler = await loadHandler();

    const response = await handler(
      new Request('https://edge.example.com/functions/v1/sessions-confirm', {
        method: 'POST',
        headers: {
          Origin: 'https://preview.example.com',
          Authorization: 'Bearer token',
          'Content-Type': 'application/json',
          'x-request-id': 'req-confirm-conflict',
          'x-correlation-id': 'corr-confirm-conflict',
          'x-agent-operation-id': 'agent-confirm-conflict',
        },
        body: JSON.stringify({
          hold_key: 'hold-1',
          session: {
