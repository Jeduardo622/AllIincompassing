...
const IDS = {
  ...
  exportedPeriodA: "90000000-0000-4000-8000-000000000105",
  exportRunA: "90000000-0000-4000-8000-000000000106",
} as const;
  ...
  try {
    ...
    await client.query(
      `insert into public.payroll_capability_grants (
         ...
         ($1::uuid, $2::uuid, 'payroll.configure_settings', '2026-08-01T00:00:00Z', $2::uuid),
...
describe.skipIf(!hasSafeLocalDatabase)("payroll administration rpc runtime contract", () => {
  ...
  it("denies broad export-only reads and redacts audit visibility for export-only and resolve-only readers", async () => {
    ...
    await admin.query(
      `insert into public.pay_periods (
         id, organization_id, pay_group_id, starts_on, ends_on, locked_at, exported_at
       ) values (
         $1::uuid, $2::uuid, $3::uuid, '2026-08-10', '2026-08-16', now(), now()
       )`,
      [IDS.exportedPeriodA, IDS.orgA, IDS.payGroupWeeklyA],
    );
    await admin.query(
      `insert into public.payroll_export_runs (
         id, organization_id, pay_period_id, pay_group_id, actor_user_id,
         canonical_hash, csv_sha256, csv_bytes, source_snapshot_count,
         source_row_count, row_count, total_regular_seconds,
         total_meal_premium_cents, total_gross_cents
       ) values (
         $1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid,
         repeat('a', 64), repeat('b', 64), convert_to('synthetic', 'utf8'), 1,
         1, 1, 3600, 7654, $6::integer
       )`,
      [
        IDS.exportRunA,
        IDS.orgA,
    ...
    const fullAdminView = await getAdministration(admin, IDS.adminA);
    expect(fullAdminView.capabilities).toEqual({
      canConfigureEmployment: true,
    ...
    expect(
      fullAdminView.payPeriods.find((period: { id: string }) => period.id === IDS.exportedPeriodA)
        ?.latestExport,
    ).toMatchObject({
      runId: IDS.exportRunA,
      totalMealPremiumCents: 7654,
      totalGrossEarningsCents: sensitiveRateCents,
    });
    await expect(getAdministration(admin, IDS.schedulerA)).rejects.toThrow(
      "payroll administration capability is required",
    );
    const resolveOnlyAdministration = await getAdministration(admin, IDS.managerA);
    expect(resolveOnlyAdministration.capabilities).toEqual({
    ...
    expect(
      resolveOnlyAdministration.payPeriods.find(
        (period: { id: string }) => period.id === IDS.exportedPeriodA,
      )?.latestExport,
    ).toBeNull();
    expect(JSON.stringify(resolveOnlyAdministration)).not.toContain("totalGrossEarningsCents");
    expect(JSON.stringify(resolveOnlyAdministration)).not.toContain(String(sensitiveRateCents));
  ...
  it("requires payroll.configure_settings for generate_periods reads and writes while leaving export capability separate", async () => {
    await executeAdministration(
      admin,
      {
        action: "set_generation_version",
      ...
      "set-version-configure-settings-authority",
      IDS.adminA,
    );
    ...
    ).rejects.toThrow(/payroll\.configure_settings capability is required/i);

    await admin.query(
      `insert into public.payroll_capability_grants (
         organization_id, user_id, capability, effective_from, granted_by
       ) values ($1::uuid, $2::uuid, 'payroll.configure_settings', '2026-08-01T00:00:00Z', $3::uuid)`,
      [IDS.orgA, IDS.schedulerA, IDS.adminA],
    );

    expect((await getAdministration(admin, IDS.schedulerA)).capabilities).toMatchObject({
      canConfigureEmployment: false,
      canGeneratePeriods: true,
    });
    await expect(
      executeAdministration(
        ...
        "configure-settings-generate-periods",
        IDS.schedulerA,
      ),
