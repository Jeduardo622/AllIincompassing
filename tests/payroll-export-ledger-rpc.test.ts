  ...
  try {
    await client.query(
      `update public.payroll_organization_settings
       set effective_from = '2026-08-01'::date
...
const supersedeOrganizationSettings = async (
  client: Client,
  options: {
    externalPayrollOrganizationId: string;
    timezone: string;
  ...
  await client.query(
    `update public.payroll_organization_settings
     set effective_through = ($2::date - 1)
     where organization_id = $1::uuid
       and effective_through is null`,
    [IDS.orgA, options.effectiveFrom],
  );
  await client.query(
    ...
    [
      IDS.orgA,
      options.externalPayrollOrganizationId,
      options.timezone,
      options.effectiveFrom,
      options.effectiveThrough ?? null,
    ],
  );
};
...
describe.skipIf(!hasSafeLocalDatabase).sequential("payroll export ledger rpc runtime contract", () => {
  ...
  it("uses the payroll settings row effective for the exported pay period instead of the newest created row", async () => {
    await supersedeOrganizationSettings(admin, {
      externalPayrollOrganizationId: "payroll-contract-org-a-future",
      timezone: "America/New_York",
      effectiveFrom: "2026-08-18",
    });
    await seedExportReadySnapshotWithTwoDatesAndRateChange(admin, "effective-dated-export-settings");

    const created = await createPayrollExport(
      admin,
      { payPeriodId: IDS.payPeriodA, adapterVersion: "provider-neutral-v1" },
      "create-payroll-export-effective-dated-settings",
      IDS.adminA,
    );
    const exportPayload = await getPayrollExport(admin, created.runId, IDS.adminA);

    expect(exportPayload.csv).toContain("payroll-contract-org-a");
    expect(exportPayload.csv).not.toContain("payroll-contract-org-a-future");
  });

  it("replays cleanly when the same actor submits the same pay-period export concurrently with the same idempotency key", async () => {
    await seedExportReadySnapshotWithTwoDatesAndRateChange(admin, "concurrent-idempotency");
    ...
    try {
      const [first, second] = await Promise.all([
        createPayrollExport(
          clientA,
          { payPeriodId: IDS.payPeriodA, adapterVersion: "provider-neutral-v1" },
          "create-payroll-export-concurrent-same-key",
          IDS.adminA,
        ),
        createPayrollExport(
          clientB,
      ...
      const replay = [first, second].find((result) => result.replayed === true);
      const created = [first, second].find((result) => result.replayed === false);
